...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        if(cryptoDevTokenReserve == 0) {
            ...
            // `liquidity` token that need to be minted to the user on `addLiquidity` call should always be proportional
           ...
           // So doing some maths, (cryptoDevTokenAmount user can add) = (Eth sent by the user * cryptoDevReserve /Eth Reserve);
    ...
    function ethToCryptoDevToken(uint _minTokens) public payable {
        ...
        // call `getAmountOfTokens` to get the amount of Crypto Dev tokens
